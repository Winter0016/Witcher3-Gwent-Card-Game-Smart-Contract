...
import {Ability, UnitType, Faction} from "./GwentTypes.sol";
...
library CardRegistryPure {
    ...
    /**
     * @notice FACTION ABILITY CATALOG (Comprehensive)
     *
     * NORTHERN REALMS (1-25):
     * - Morale_Boost(1), Tight_Bond(4), Spy(3), Medic(1), Hero(4).
     *
     * SCOIA'TAEL (26-48):
     * - Muster(3), Medic(2), Morale_Boost(2), Hero(4).
     *
     * NILFGAARD (49-77):
     * - Medic(2), Tight_Bond(3), Spy(3), Hero(4).
     *
     * MONSTER (78-112):
     * - Muster(13), Hero(4).
     *
     * SKELLIGE (113-135):
     * - Berserker(3), Commander_horn(1), Tight_Bond(4), Mardroeme(2), Morale_Boost(1), Hero(3).
     *
     * NEUTRAL (136-144):
     * - Mardroeme(1), Commander_horn(1), Decoy(1), Scorch(1), Weather_Sets(5).
     */
    ...
    function getCard(uint16 id) public pure returns (Card memory) {
        ...
        return Card(0, 0, Ability.None, UnitType.Close_Combat, Faction.Neutral);
    }

    function getMusterMembersByID(
        uint16 id
    ) external pure returns (uint256 packed) {
        //16 bits distance

        if (id < 89) {
            if (id == 78) return (1 | (78 << 8)); // ranged
            if (id == 85) return (1 | (85 << 8)); // ranged
            if (id == 87) return (2 | (87 << 8) | (101 << 24)); // close combat( amount : 2)
            if (id == 28) return (1 | (28 << 8)); // ranged
            if (id == 31) return (1 | (31 << 8)); // ranged
            if (id == 34) return (1 | (34 << 8)); // ranged
        } else {
            if (id == 101) return (2 | (87 << 8) | (101 << 24)); // already checked
            if (id >= 102 && id <= 104)
                return (3 | (102 << 8) | (103 << 24) | (104 << 40)); // siege (amount : 3)
            if (id == 89 || id == 90 || id == 91 || id == 92 || id == 99) {
                // close combat (amount : 5)
                return (5 |
                    (89 << 8) | // power 4 each
                    (90 << 24) |
                    (91 << 40) |
